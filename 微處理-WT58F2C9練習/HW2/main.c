/*
include math.h
 需要在專案的Build Settings中
 點選C/C++ Build -> Settings -> Andes C Linker -> Libraries -> Libraries (-l)中加入"m"
參考網址 : http://forum.andestech.com/viewtopic.php?f=23&t=177
*/
...
int main()
{
	unsigned int left=0x0000,right=0x0000,mark=0x10000;
	...
	for(k=0;k<8;k++)
	{
		...
		{
			mark = mark >> 1;
			GPIO_PTB_GPIO = ~(left | mark | right);
			delay1(200000);
		}
		rightCount++;
		right = pow(2,rightCount)-1;
		for(i=0; i<(16-2*k-1); i++)
		{
			mark = mark << 1;
			GPIO_PTB_GPIO = ~(left | mark | right);
			delay1(200000);
	...
	for(k=0;k<8;k++)
	{
		...
		{
			mark = mark >> 1;
			GPIO_PTB_GPIO = (~(left | right) | mark);
			delay1(200000);
		}
		right -= pow(2,rightCount);
		rightCount++;
		for(i=0; i<(16-2*k-1); i++)
		{
			mark = mark << 1;
