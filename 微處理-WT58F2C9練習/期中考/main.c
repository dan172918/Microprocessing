...
int main()
{
	...
	unsigned int tmp = 0,tmp1 = 0,i,count = 0,V1=0,V0=0,Result=0,myi,k,j,m,lastState = 0x0FFF;
	...
	unsigned int NumArr[4]={~(Number_8|Number_Dot),~(Number_8|Number_Dot),~(Number_8|Number_Dot),~(Number_8|Number_Dot)};
	unsigned int ResultArr[4]={Number_0,~(Number_8|Number_Dot),~(Number_8|Number_Dot),~(Number_8|Number_Dot)};
	/*7-SEG LED顯示位置&key switch 初始設定*/
	GPIO_PTA_DIR = 0x0000;
	GPIO_PTA_CFG = 0x0000;
	GPIO_PTA_GPIO = Digit_8;
	/*LED初始設定*/
	GPIO_PTB_DIR = 0x0000;	//設定為output
	GPIO_PTB_CFG = 0xFFFF;
	GPIO_PTB_PADIN = 0x00;
	/*DIP SW初始設定*/
	GPIO_PTC_FS = 0x0000;
	GPIO_PTC_PADINSEL = 0x0000;
	GPIO_PTC_DIR = 0xFFFF;	//設定為input
	GPIO_PTC_CFG = 0x0000;
	/*7-SEG LED初始設定*/
	GPIO_PTD_DIR = 0x0000;
	GPIO_PTD_CFG = 0x0000;
	...
	while(1)
	{
		...
		tmp1 = tmp = (GPIO_PTC_PADIN >> 2) & 0x000000FF;
		/*把 (GPIO_PTC_PADIN >> 2) & 0x000000FF; 翻轉*/
		//舉例 : 1100 0010 -> 11 00 00 01 先分為四組，每組中的數字換位置
		tmp =((tmp & 0x0055)<<1) | ((tmp & 0x00AA)>>1);
		// 1100 0001 -> 0011 0100 再來是{第8,第7}和{第6,第5}換位置           {第3,第2}和{第1,第0}換位置
		tmp =((tmp & 0x0033)<<2) | ((tmp & 0x00CC)>>2);
		// 0011 0100 -> 0100 0011 以4個數字為一組，並換位置 -> {第8,第7,第6,第5} 和  {第3,第2,第1,第0}
		tmp =((tmp & 0x000F)<<4) | ((tmp & 0x00F0)>>4);
		//(tmp<<8) ^ (~tmp & 0x00FF) 因為0~7和8~15是相反結果，所以有一側要not
		//要達到這個結果0100 0011 1011 1100 = 0100 0011 0000 0000 ^ ~(0000 0000 0100 0011) = 0100 0011 0000 0000 ^ 1111 1111 1011 1100
		tmp = (tmp<<8) ^ (~tmp & 0x00FF);
		GPIO_PTB_GPIO = tmp;
		/*lastState!=tmp1 : 如果跟上一次開關不一樣*/
		if(lastState!=tmp1)
		{
			/*陣列初始*/
			ResultArr[3]=ResultArr[1]=ResultArr[2]=NumArr[0]=NumArr[1]=NumArr[2]=NumArr[3]=~(Number_8|Number_Dot);
			ResultArr[0]=Number_0;
			//取V0和V1值
			V0 = tmp1 & 0x0F;
			V1 = (tmp1 & 0xF0) >> 4;
			Result = V1*V0*(V1+V0);
			/*把V1和V0分別存入陣列*/
			if(V1 < 10)
				NumArr[1] = index_7LED_NUM[V1];
			else
			{
				NumArr[1] = index_7LED_NUM[V1%10];
				NumArr[0] = index_7LED_NUM[(V1/10)%10];
			}
			if(V0 < 10)
				NumArr[3] = index_7LED_NUM[V0];
			else
			{
				NumArr[3] = index_7LED_NUM[V0%10];
				NumArr[2] = index_7LED_NUM[(V0/10)%10];
			}
			/*結果放入陣列*/
			i= 0;
			while(Result!=0)
			{
				ResultArr[i] = index_7LED_NUM[Result%10];
				Result/=10;
				i++;
			}
		}
		/*印出LED左旋 先保留最低位，再補到最高位*/
		if(count == 0)
		{
			for(i=0;i<17;i++)
			{
				GPIO_PTB_GPIO = tmp;
				tmp = (tmp >> 1) | (tmp & 0x0001)<<15;
				delay1(500000);
			}
			count++;
		}
		/*如果是新輸入的，執行跑馬效果，否則直接印出*/
		if(lastState!=tmp1)
		{
			lastState = tmp1;
			for(i=0; i<4; i++)
			{
				for(j=0; j<4-i; j++)
				{
					for (myi=0; myi<200; myi++)
					{
						GPIO_PTA_GPIO = index_7LED[3-j];
						GPIO_PTD_GPIO = ResultArr[i];
						delay1(500);
						for(k=0; k<i; k++)
						{
							GPIO_PTA_GPIO = index_7LED[k];
							GPIO_PTD_GPIO = ResultArr[k];
							delay1(500);
						}
						for(m=0; m<4; m++)
						{
							GPIO_PTD_GPIO = NumArr[m];
							GPIO_PTA_GPIO = index_7LED[7-m];
							delay1(500);
						}
					}
				}
