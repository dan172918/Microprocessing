...
int main()
{
	...
	while(1){
		tmp = (GPIO_PTC_PADIN >> 2) & 0x0000000F; //PC2開始 0F只取4個開關
		/*第一個方式開始*/
		if (tmp == 0x00000001){
			n=7; //n表示每次起始位置
			for(i=0; i<8; i++){
			...
			/*判斷目前開關是否與之前一樣  ( 0x00000001 ) ? 重複印出 : 跳出迴圈 */
			while(((GPIO_PTC_PADIN >> 2) & 0x0000000F) == 0x00000001)
			{
				for(j=0; j<=7; j++){
		...
		/*第二個方式開始*/
			...
			/*判斷目前開關是否與之前一樣  ( 0x00000002 ) ? 重複印出 : 跳出迴圈 */
			while(((GPIO_PTC_PADIN >> 2) & 0x0000000F) == 0x00000002)
			{
				for(j=0; j<=7; j++){
		...
		/*第三個方式開始*/
			...
			for(i=0; i<8; i++){
				for(j=0; j<8-i; j++){
					for (myi=0; myi<200; myi++){
						GPIO_PTA_GPIO = index_7LED[7-j];	/*印出正在跑的數字*/
						GPIO_PTD_GPIO = index_7LED_NUM[myId[8-i]];
						delay1(500);
						for(k=0; k<i; k++){
							GPIO_PTA_GPIO = index_7LED[k];	/*印出到位置的數字*/
			...
			/*判斷目前開關是否與之前一樣  ( 0x00000004 ) ? 重複印出 : 跳出迴圈 */
			while(((GPIO_PTC_PADIN >> 2) & 0x0000000F) == 0x00000004)
			{
				for(j=0; j<=7; j++){
					GPIO_PTA_GPIO = index_7LED[j];
					GPIO_PTD_GPIO = index_7LED_NUM[myId[8-j]];
		...
		/*第四個方式開始*/
			...
			for(i=0; i<8; i++){
				for(j=0; j<8-i; j++){
					for (myi=0; myi<200; myi++){
						GPIO_PTA_GPIO = index_7LED[j];	/*印出正在跑的數字*/
						GPIO_PTD_GPIO = index_7LED_NUM[myId[i]];
						delay1(500);
						for(k=0; k<i; k++){
							GPIO_PTA_GPIO = index_7LED[7-k];	/*印出到位置的數字*/
			...
			/*判斷目前開關是否與之前一樣  ( 0x00000008 ) ? 重複印出 : 跳出迴圈 */
			while(((GPIO_PTC_PADIN >> 2) & 0x0000000F) == 0x00000008)
			{
				for(j=0; j<=7; j++){
					GPIO_PTA_GPIO = index_7LED[7-j];
					GPIO_PTD_GPIO = index_7LED_NUM[myId[j]];
