...
int main()
{
	...
	while(1)
	{
		...
		for (col=0; col<4; col++)
		{
			...
			tmp = ((~GPIO_PTA_PADIN) & 0xFF0) >> 4;	//取輸入部分
			if (tmp > 0)
			{
				if (tmp & 0x1)
					key = 0*4 + col;
		...
		if(key == 0)	//按鍵1顯示第一組字
		{
			WriteIns(0x01);	//清除所有，並初始
			lastState = key;
			Display_1Line(80);	//共80個字
			delay1(1000000);
		}
		else if(key == 1)	//按鍵2顯示第一組字
		{
			WriteIns(0x01);	//清除所有，並初始
			lastState = key;
			Display_2Line(80);	//共80個字
			delay1(1000000);
		}
		else if(key == 2)	//按鍵3左移
		{
			...
			for(i=0;i<24;i++)
			{
				WriteIns(0x18);	//0001 1000，S/C = 1 = 整個移動 , R/L = 0 = 往左移
				delay1(100000);
			}
		}
		else if(key == 3)	//按鍵4右移
		{
			...
			for(i=0;i<24;i++)
			{
				WriteIns(0x1C);	//0001 1100，S/C = 1 = 整個移動 , R/L = 1 = 往右移
				delay1(100000);
			}
		}
		else if(key == 4)	//按鍵5 Clear Display
		{
			lastState = key;
			WriteIns(0x01);	//清除所有，並初始
		}
		else if(key == 5)	//按鍵6 Return Home
		{
			lastState = key;
			WriteIns(0x02);	//Cursor跑到左上角
		}
		else if(key == 6 && lastState!=key)	//按鍵7 Cursor on/off
		{
			lastState = key;
			if(onCursor == 1)
			{
				WriteIns(0x0C);//0000 1100，D(LCM顯示) = 1,C(Cursor顯示) = 0,B(Cursor閃爍) = 0
				onCursor = 0;
			}
			else
			{
				WriteIns(0x0E);//0000 1110，D(LCM顯示) = 1,C(Cursor顯示) = 1,B(Cursor閃爍) = 0
				onCursor = 1;
			}
		}
		else if(key == 7)	//按鍵8 測試
		{
			lastState = key;
			setCursor(1,3);
		}
